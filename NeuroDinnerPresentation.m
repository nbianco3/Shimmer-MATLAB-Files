    ...
    if (shimmer1.start)
        ...
        while (elapsedTime < captureDuration)
            ...
            if ~isempty(calDataShimmer1)
                ...
                % Plot ExG data
                subplot(2,2,4)
                plot((emgDataShimmer1(:,1)-mean(emgDataShimmer1(:,1))/max(emgDataShimmer1(:,1))))
                keyboard
                hold on
                plot(emgFiltDataShimmer1(:,1)/max(emgFiltDataShimmer1(:,1)));                                                             
                axis([0 numPlotSamples -1 1]);
                ylabel('Muscle Excitation (mV)')
